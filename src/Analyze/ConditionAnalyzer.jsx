...
const getCountry = (patient) => {
  if (patient.address && patient.address.length > 0 && patient.address[0].country) {
...
const ConditionAnalyzer = ({ conditionName, onDataFetched }) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const patientConditionPairs = filterPatientsByCondition(conditionData, conditionName);
        ...
        const demographicsResult = analyzeDemographics(patientConditionPairs, patientData);
        const medicationsResult = analyzeMedications(patientConditionPairs, patientData, medicationRequestData);
  ...
  const filterPatientsByCondition = (conditionData, conditionName) => {
    ...
    return Array.from(
      new Map(patientConditionPairs.map((pair) => [`${pair.patientId}-${pair.conditionId}`, pair])).values()
    );
  };
  ...
  const analyzeDemographics = (patientConditionPairs, patientData) => {
    ...
    patientConditionPairs.forEach(({ patientId }) => {
      ...
      if (ageVal !== null) {
        ages.push(ageVal);
        if (ageVal <= 18) age.distribution["0-18"] = (age.distribution["0-18"] || 0) + 1;
        else if (ageVal <= 35) age.distribution["19-35"] = (age.distribution["19-35"] || 0) + 1;
        else if (ageVal <= 50) age.distribution["36-50"] = (age.distribution["36-50"] || 0) + 1;
        else if (ageVal <= 65) age.distribution["51-65"] = (age.distribution["51-65"] || 0) + 1;
    ...
    age.averageAge = ages.length ? ages.reduce((sum, age) => sum + age, 0) / ages.length : 0;
    age.medianAge = sortedAges.length ? sortedAges[Math.floor(sortedAges.length / 2)] : 0;

    return { age, gender, country };
  };

  const analyzeMedications = (patientConditionPairs, patientData, medicationRequestData) => {
    ...
    medicationRequestData.forEach((med) => {
      ...
      const { subject, medicationCodeableConcept, reasonReference } = med.entry[0].resource;
      const patientId = subject.reference.split("/")[1];

      const matchedCondition = patientConditionPairs.find(
        (pair) => pair.patientId === patientId &&
          reasonReference?.some((ref) => ref.reference === `Condition/${pair.conditionId}`)
      );
      ...
      const medName = medicationCodeableConcept?.coding?.[0]?.display || "Unknown";
      ...
      if (!medicationDistribution[medName]) {
        medicationDistribution[medName] = {
          ageGroups: { "0-18": 0, "19-35": 0, "36-50": 0, "51-65": 0, "66+": 0 },
          genderDistribution: { male: 0, female: 0, non_binary: 0, prefer_not_to_say: 0 },
          countryDistribution: {},
        };
      }
    ...
    return Object.entries(medicationDistribution).map(([name, stats]) => ({
      ...
      countryDistribution: Object.entries(stats.countryDistribution).map(([name, population]) => ({ name, population }))
    }));
  };
